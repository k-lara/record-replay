...
public class StudyProcedureSteps : MonoBehaviour
{
    ...
    private string mirrorInstructions = "Before every recording, you will see your current avatar in the mirror. \n" +
                                        "You can check if the tracking works as expected! \n" +
                                        "Here, you will also get the character description for this recording and what you are supposed to do! \n" +
    ...
    private string nextRecording = "Recording successful! :) \n" +
                                   "Press the NEXT button to teleport to the next location and change your avatar!";
    ...
    private string takeoverAvatarRun1 = "Once you press the REDO button, you will takeover a previously recorded character. \n" +
                                    "You will get the same acting instructions again for this character.\n" +
                                    "This time, all other characters are already in the scene and you can react to them directly.";

    private string takeoverAvatarRun2 = "Like before, press the REDO button and you will takeover a character. \n" +
                                        "The acting instructions for the character will be the same.";
    
    private string nextTakeover = "Recording successful! :) \n" +
                                  "Press the REDO button to takeover the next character!";
    ...
    private string scenario2Introduction = "Scenario 2: ...";
    private string scenario3Introduction = "Scenario 3: ...";
    
    // TODO: face in the direction of the arrows on the floor
    private string s1c0 = "Character 1 (name?): ...";
    private string s1c1 = "Character 2 (name?): ...";
    private string s1c2 = "Character 3 (name?): ...";
    private string s1c3 = "Character 4 (name?): ...";
    private string s1c4 = "Character 5 (name?): ...";
    
    private string s2c0 = "Character 1 (name?): ...";
    private string s2c1 = "Character 2 (name?): ...";
    private string s2c2 = "Character 3 (name?): ...";
    private string s2c3 = "Character 4 (name?): ...";
    private string s2c4 = "Character 5 (name?): ...";
    
    private string s3c0 = "Character 1 (name?): ...";
    private string s3c1 = "Character 2 (name?): ...";
    private string s3c2 = "Character 3 (name?): ...";
    private string s3c3 = "Character 4 (name?): ...";
    private string s3c4 = "Character 5 (name?): ...";
    ...
    public IEnumerator Scenario(Scenario scenario, string scenarioIntro, string c0, string c1, string c2, string c3, string c4)
    {
        ...
        if (!studyUI.recordingManager.CheckIfSaveExists("Run1"))
        {
        ...
        {
            if (!studyUI.recordingManager.CheckIfSaveExists("Run2"))
            {
            ...
            {
                if (!studyUI.recordingManager.CheckIfSaveExists("Run3"))
                {
                    yield return LoadRecording(scenario);
    ...
    public IEnumerator NewOrResumeUser()
    {
        ...
        else if (studyUI.resumeUserPressed)
        {
            ...
            studyUI.recordingManager.PreparePreviousUserFolder();
    ...
    public IEnumerator Run1(Scenario scenario, string c0, string c1, string c2, string c3, string c4)
    {
        ...
        maxRecordingTime = studyUI.maxReplayTime;
    ...
    public IEnumerator PositionMirrorRecordNext(Scenario scenario, int spawnPoint, string characterDescription,
        bool hasNext = true)
    {
        yield return SetSpawnPointAndAvatar(scenario, spawnPoint);
        yield return ShowMirrorAndCharacterDescription(characterDescription);

        yield return Recording(); // does the recording and redo if necessary
    ...
    public IEnumerator SetSpawnPointAndAvatar(Scenario scenario, int index)
    {
    ...
    public IEnumerator TakeoverMirrorRecordNext(GameObject takeoverPrefab, int takeoverIndex, 
        string characterDescription, bool hasNext = true)
    {
        yield return WaitRedoPressed(takeoverPrefab, takeoverIndex);
        yield return ShowMirrorAndCharacterDescription(characterDescription);
    ...
    public IEnumerator TakeoverRun(Scenario scenario, string takeoverRun, string c0, string c1, string c2, string c3, string c4)
    {
        ...
        yield return TakeoverMirrorRecordNext(scenario.AvatarPrefabs[0], scenario.NumberBaseAvatars, c0);
        
        yield return TakeoverMirrorRecordNext(scenario.AvatarPrefabs[1], scenario.NumberBaseAvatars + 1, c1);
        
        yield return TakeoverMirrorRecordNext(scenario.AvatarPrefabs[2], scenario.NumberBaseAvatars + 2, c2);
        
        yield return TakeoverMirrorRecordNext(scenario.AvatarPrefabs[3], scenario.NumberBaseAvatars + 3, c3);

        yield return TakeoverMirrorRecordNext(scenario.AvatarPrefabs[4], scenario.NumberBaseAvatars + 4, c4, false);
    }
    ...
    public IEnumerator Recording()
    {
        ...
        yield return WaitRecordPressed();
     ...
     */
    public IEnumerator RedoIfWanted()
    {
        yield return UIToggle(true);
        ...
        yield return WaitRedoRecordingOrNext(studyProcedure.scenario1.AvatarManager.avatarPrefab);
    }

    // used when tracking failed!
    public IEnumerator RedoRecording()
    {
        studyUI.instructionsText.text = trackingLost;
        yield return UIToggle(true);

        yield return WaitRedoPressed(studyProcedure.scenario1.AvatarManager.avatarPrefab);
    ...
    {
        yield return MirrorToggle(true);
        studyUI.instructionsText.text = mirrorInstructions;
    ...
    public IEnumerator UIToggle(bool uiVisible)
    {
        ...
        if (cameraMain && uiVisible)
        {
            // align it with the direction the cameraMainera is looking at
            studyUI.gameObject.transform.rotation = Quaternion.LookRotation(cameraMain.transform.forward);
            studyUI.gameObject.transform.position = 
                new Vector3(cameraMain.transform.position.x, cameraMain.transform.position.y - 0.2f, cameraMain.transform.position.z) + 
                cameraMain.transform.forward * 0.45f;
        }
    ...
    public IEnumerator WaitRecordPressed()
    {
    ...
    public IEnumerator WaitRedoPressed(GameObject takeoverAvatar, int n = -1)
    {
    ...
    public IEnumerator WaitRedoRecordingOrNext(GameObject takeoverAvatar)
    {
        studyUI.redoSphere.gameObject.SetActive(true);
        studyUI.nextSphere.gameObject.SetActive(true);
